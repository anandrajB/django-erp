...
function new_pf(){
    ...
    //     }
    // });
    common_drop_down_get('product',`form_product`,'product_name')
...
function get_part (element) {
    ...
    fetch( `${host}/api/pf_dropdown?id=${form_product}&&filter=yes`,{
        method: 'GET',
        headers: {
...
function postgetpf(event){
    ...
    if(event=='edit'){
     ...
     phase_list = []
        for(i=0;i<phase_data.length;i++){
            ...
            select = `<select onchange=change_field('edit,'phase',this,${i}) id='form_phase'><option value=''>------</option>`
                    ...
                    tr += `<td>${select}</td>`
                    tr += `<td><input type='number' onchange=change_field('edit','quantity_perday',this,${1}) value='${phase_data[i].quantity_perday}' id='form_productivity'></input></td>
                    `
                    tr += `<td><input type='number' onchange=change_field('edit','scrap_quantity',this,${1}) value='${phase_data[i].scrap_quantity}' id='form_scrap'></input></td>`
    ...
    select = `<select onchange=change_field(null,'phase',this,${0}) id='form_phase'> <option value=''>------</option>`
    ...
    table += `<tbody><tr id='input'><td>${select}</td><td><input onchange=change_field(null,'quantity_perday',this,${0}) type='number' id='form_productivity'></input></td>
    <td><input type='number' onchange=change_field(null,'scrap_quantity',this,${0}) id='form_scrap'></input></td><td>${button}</td></tr></tbody>`
    table += `</table>`
    // if(show){
...
//     }
// });
  fetch( `${host}/api/dropdown/?model=productionphase&&name=phase_name`,{
    method: 'GET',
    headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json',
...
// common_drop_down_get('productionphase',`id_party_type`,'partytype')
...
if(index==0){
    if(event && event == 'edit'){
        phases_to_edit.push(phases_to_change)
    }
    else{
        phases.push(phases_to_change)
    }
}else{
        if(event && event == 'edit'){
            phases_to_edit[index] = phases_to_change
        }else{
            phases[index] = phases_to_change
        }
    }
    // create_table_form_pf()
}